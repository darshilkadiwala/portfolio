...
const getMonthLabels = (weeks: Week[], monthNames: string[] = DEFAULT_MONTH_LABELS): MonthLabel[] => {
  return weeks
    .reduce<MonthLabel[]>((labels, week, weekIndex) => {
      ...
      if (!firstActivity) {
        throw new Error(`Unexpected error: Week ${weekIndex + 1} is empty: [${JSON.stringify(week)}].`);
      }
...
export const ContributionGraphLegend = ({ className, children, ...props }: ContributionGraphLegendProps) => {
  ...
  return (
    <div className={cn('ms-auto flex items-center gap-0.75', className)} {...props}>
      <span className='text-muted-foreground me-1'>{labels.legend?.less || 'Less'}</span>
      ...
      <span className='text-muted-foreground ms-1'>{labels.legend?.more || 'More'}</span>
    </div>
  );
};
